    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-xl">
        ...
        <div className="px-6 py-4 border-b border-neutral-200 flex justify-between items-center bg-neutral-50">
          ...
          <button onClick={onClose} disabled={isSubmitting} className="text-neutral-400 hover:text-black font-bold text-xl disabled:opacity-50 transition-colors">
            &times;
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          {loading ? (
             <div className="py-8 text-center text-black font-bold animate-pulse">Cargando datos del equipo...</div>
          ) : (
            <>
              {/* Controles para agregar */}
              <div className="flex gap-3 items-end">
                <div className="flex-1 space-y-1">
                  <label className="text-sm font-bold text-black">Seleccionar alumno</label>
                  <select 
                    className="w-full border border-neutral-300 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-black focus:border-black transition-all bg-white text-black"
                ...
                <button 
                  ...
                  className="px-4 py-2 bg-black text-white rounded-xl hover:bg-neutral-800 transition-transform active:scale-95 whitespace-nowrap h-[42px] disabled:opacity-50 disabled:cursor-not-allowed font-bold"
                >
                  Agregar
                </button>
              </div>
              ...
              <div className="space-y-2">
                <h4 className="text-sm font-bold text-black">Integrantes actuales</h4>
                <div className="border border-neutral-200 rounded-xl divide-y divide-neutral-100 max-h-48 overflow-y-auto">
                  {integrantes.length === 0 ? (
                    <p className="text-sm text-black font-medium p-4 text-center italic">No hay integrantes en este equipo.</p>
                  ) : (
                    integrantes.map(integrante => (
                      <div key={integrante.id_usuario} className="flex justify-between items-center p-3 hover:bg-neutral-50 transition-colors">
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-black">{integrante.usuario?.nombre || integrante.nombre || 'Sin nombre'}</span>
                          <span className="text-xs text-black">{integrante.num_ctrl}</span>
                        </div>
                        <button 
        ...
        <div className="px-6 py-4 border-t border-neutral-200 flex justify-end bg-neutral-50">
          <button 
            ...
            className="px-6 py-2 bg-black text-white rounded-xl hover:bg-neutral-800 transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed font-bold"
          >
